...
OPA_ENVOY_DIR=/go/src/github.com/rmstorm/opa-envoy-plugin
...
clone_repo() {
    git clone $SOURCE_URL /go/src/github.com/rmstorm/opa-envoy-plugin
    cd /go/src/github.com/rmstorm/opa-envoy-plugin
    if [ -n "$VERSION" ]; then
